  ...
  if ((state->dragrect==sim->ui0.rectid) ||
    ...
    // simulation section
      ...
      case 1:             // Reset
        ...
        sim->pending_.reloadPhysics = true;
        break;
      ...
      case 4:             // Reload Controller
        sim->pending_.reloadController = true;
        break;
...
#define X(name)                                                  \
  ...
  if (pending_.reloadPhysics) { /*** AUVC ***/
    this->reloadPhysicsLibPlug(m_,d_,AUVC_DEBUG,true);
    pending_.reloadPhysics = false;
  }

  if (pending_.reloadController) { /*** AUVC ***/
    this->reloadControllerLibPlug(m_,d_,AUVC_DEBUG,true);
    pending_.reloadController = false;
  }
...
bool Simulate::reloadControllerLibPlug(mjModel* m, mjData* d, bool test, bool load_dll){
    ...
    libcontroller = dlopen(libcontroller_filename, RTLD_NOW);
    if (libcontroller == NULL){
        fprintf(stderr, "ERROR: (%d) could not load %s: %s\n", __LINE__, libcontroller_filename, dlerror());
        return false;
    }
    plug_controller_test = (plug_test_t)dlsym(libcontroller, "controllerTestFunc");
    if (plug_controller_test == NULL){
        fprintf(stderr, "ERROR: (%d) could not load symbol test_func() from %s: %s\n", __LINE__, libcontroller_filename, dlerror());
        dlclose(libcontroller);
        return false;
    }
    if(test) {plug_controller_test();}
    plug_controller_init = (plug_init_t)dlsym(libcontroller, "controllerInitPlug");
    if (plug_controller_init == NULL){
        fprintf(stderr, "ERROR: (%d) could not load symbol initPlug() from %s: %s\n", __LINE__, libcontroller_filename, dlerror());
        dlclose(libcontroller);
        return false;
    }
    if(test) {plug_controller_init(m,d);}
    plug_controller_update = (plug_update_t)dlsym(libcontroller, "controllerUpdatePlug");
    if (plug_controller_update == NULL){
        fprintf(stderr, "ERROR: (%d) could not load symbol updatePlug() from %s: %s\n", __LINE__, libcontroller_filename, dlerror());
        dlclose(libcontroller);
        return false;
    }
    if(test) {plug_controller_update(m,d);}
    if(load_dll){
      MutexLock lock(mtx);
      printf("Loading Controller Code\n");

      this->reload_plug_bool = 0;
      this->pending_.reloadController = false;
      cond_reload_plug_bool.notify_all();

      cond_reload_plug_bool.wait(lock, [this]() { return this->reload_plug_bool == 0; });
      printf("Sucessfully Loaded Controller Code\n");
    }
    return true;
}

/*** AUVC ***/
bool Simulate::reloadPhysicsLibPlug(mjModel* m, mjData* d, bool test, bool load_dll){
    if(libphysics != NULL) {dlclose(libphysics);}

    libphysics = dlopen(libphysics_filename , RTLD_NOW);
    if (libphysics == NULL){
        fprintf(stderr, "ERROR: (%d) could not load %s: %s\n", __LINE__, libphysics_filename, dlerror());
        return false;
    }
    plug_physics_test = (plug_test_t)dlsym(libphysics, "roverPhysicsTestPlug");
    if (plug_physics_test == NULL){
        fprintf(stderr, "ERROR: (%d) could not load symbol test_func() from %s: %s\n", __LINE__, libphysics_filename, dlerror());
        dlclose(libphysics);
        return false;
    }
    if(test) {plug_physics_test();}
    plug_physics_init = (plug_init_t)dlsym(libphysics, "roverPhysicsInitPlug");
    if (plug_physics_init == NULL){
        fprintf(stderr, "ERROR: (%d) could not load symbol initPlug() from %s: %s\n", __LINE__, libphysics_filename, dlerror());
        dlclose(libphysics);
        return false;
    }
    if(test) {plug_physics_init(m,d);}
    plug_physics_update = (plug_update_t)dlsym(libphysics, "roverPhysicsUpdatePlug");
    if (plug_physics_update == NULL){
        fprintf(stderr, "ERROR: (%d) could not load symbol updatePlug() from %s: %s\n", __LINE__, libphysics_filename, dlerror());
        dlclose(libphysics);
        return false;
    }
    if(test) {plug_physics_update(m,d);}
    if(load_dll){
      MutexLock lock(mtx);
      printf("Loading Physics Code\n");

      // clear request
      this->reload_plug_bool = 0;
      this->pending_.reloadPhysics = false;
      cond_reload_plug_bool.notify_all();

      cond_reload_plug_bool.wait(lock, [this]() { return this->reload_plug_bool == 0; });
      printf("Sucessfully Loaded Physics Code\n");
    }
    return true;
}
...
void Simulate::Render() {
  ...
  /*** AUVC ***/
  rect = this->uistate.rect[3];
  mjrRect const_viewport = {
