...
class Simulate {
 public:
  ...
  /*** AUVC ***/
  bool reloadControllerLibPlug(mjModel* m, mjData* d, bool test, bool load_dll);
  bool reloadPhysicsLibPlug(mjModel* m, mjData* d, bool test, bool load_dll);
  ...
  int color = 0;
  int font = 0;
  int ui0_enable = 0; // 1 // Modify these values /* AUVC */
  int ui1_enable = 0; // 1
  int help = 0;
  int info = 0;
  int profiler = 0;
  int sensor = 0;
  int reload_controller = 0;
  int reload_physics = 0;
  int sub_camera = 1; // 1
  int pause_update = 0;
  int fullscreen = 0;
  int vsync = 1;
  ...
  #endif
    {mjITEM_CHECKINT, "SubCAM",        2, &this->sub_camera, " #295"}, /*** AUVC F6 ***/
  ...
  const mjuiDef def_simulation[15] = {
    ...
    {mjITEM_BUTTON,    "ReloadControl", 2, nullptr,              "CH"},
